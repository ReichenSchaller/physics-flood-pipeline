...
def _read_open_boundary_outline_manifest(manifest_path: str) -> tuple[dict[str, Any], list[str]]:
    """
    ...
    if not path.exists():
        return {}, [f"Open-boundary outline manifest does not exist: {path}"]

    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        return {}, [f"Could not read open-boundary outline manifest {path}: {exc}"]

    raw_recommended = data.get("recommended_config") or {}

    if not isinstance(raw_recommended, dict):
        return {}, [f"Open-boundary outline manifest recommended_config is not an object: {path}"]
    ...
    outline_path = (
        suggested.get("open_boundary_outline_path")
        or data.get("default_open_boundary_outline_path")
        or ""
    )

    outline_path = str(outline_path).strip()

    if outline_path:
        outline = Path(outline_path)

        if not outline.is_absolute():
            outline = (path.parent / outline).resolve()

        suggested["open_boundary_outline_path"] = str(outline)

        if not outline.exists():
            warnings.append(f"Open-boundary outline path from manifest does not exist: {outline}")

    if suggested.get("open_boundary_outline_path"):
        suggested.setdefault("open_boundary_mask_mode", "coastal_outline")
        suggested.setdefault("open_boundary_outline_buffer_m", 150.0)
        suggested.setdefault("open_boundary_min_cells", 50)
        suggested.setdefault("open_boundary_max_cells", 500)
        suggested.setdefault("open_boundary_require_edge_adjacency", True)
        suggested.setdefault("open_boundary_allow_overwrite_special", False)
...
def detect_data_catalogs_from_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    open_boundary_manifest_path = choose_found("open_boundary_outline_manifest_path")

    if open_boundary_manifest_path:
        open_boundary_suggestions, open_boundary_warnings = _read_open_boundary_outline_manifest(
            open_boundary_manifest_path
        )
        suggested_config.update(open_boundary_suggestions)
        warnings.extend(open_boundary_warnings)
    else:
        open_boundary_outline_path = choose_found("open_boundary_outline_path")

        if open_boundary_outline_path:
            suggested_config["open_boundary_mask_mode"] = "coastal_outline"
            suggested_config["open_boundary_outline_path"] = open_boundary_outline_path
            suggested_config["open_boundary_outline_buffer_m"] = 150.0
            suggested_config["open_boundary_min_cells"] = 50
            suggested_config["open_boundary_max_cells"] = 500
            suggested_config["open_boundary_require_edge_adjacency"] = True
            suggested_config["open_boundary_allow_overwrite_special"] = False
    ...
    for p in sorted(run_root.iterdir(), key=lambda x: x.stat().st_mtime if x.exists() else 0, reverse=True):
        ...
        try:
            ...
            rows.append({
                ...
                "mtime": st.st_mtime,
                "mtime_iso": __import__("datetime").datetime.fromtimestamp(st.st_mtime, __import__("datetime").UTC).replace(microsecond=0).isoformat(),
            })
        except Exception as exc:

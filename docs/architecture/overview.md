...
flowchart TB
    ...
    Settings["Site settings and<br/>allowed roots"]

    subgraph Launcher["Web Launcher"]
        direction TB
        Guide["Built-in user guide"]
        Pages["HTML pages"]
        Checks["JavaScript interface<br/>and checks"]

        Guide -.-> Pages
        Pages --> Checks
    end

    Flask["Flask application<br/>and API"]
    Config["Structured run<br/>configuration"]

    Static["Static data<br/>catalogs"]
    Event["Event data<br/>catalogs"]
    Native["Optional trusted<br/>SFINCS-native inputs"]

    Backend["Python pipeline backend"]

    subgraph Slurm["Slurm Execution Stack"]
        direction TB
        Preprocess["Preprocessing job"]
        Solver["SFINCS solver job"]
        Postprocess["Postprocessing job"]

        Preprocess --> Solver
        Solver --> Postprocess
    end

    RunFolder["Run directory"]
    Outputs["Model outputs, logs,<br/>and provenance"]

    Review["Review Mode"]
    Validation["Scientific validation<br/>and interpretation"]
    ...
    Preprocess --> RunFolder
    Solver --> RunFolder
    Postprocess --> RunFolder
    ...
    classDef researcher fill:#1f4e79,stroke:#102f4d,color:#ffffff,stroke-width:3px
    class Researcher researcher

    style Launcher fill:#fffde3,stroke:#c8bd55
    style Slurm fill:#fffde3,stroke:#c8bd55
```

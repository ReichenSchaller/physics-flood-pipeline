...
## A. Active hardcoded data path chunks — full
...
### `/proj/zefflab/projects/Flooding/Validation/Code/raw_dem_depth_correlation_graph_generator.py`

Hits: **2**

#### Hit 1/2 — line 61 — `LIKELY_DEFAULT_OR_BROWSE`

Current line:

```text
EVENTS_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events")
```

Simple root replacement would become:

```text
EVENTS_ROOT = Path("/users/e/p/epsilon/Data/Data/harris_county/catalogs/events")
```
...
```text
   00049: 
   00050: import rasterio
   00051: from rasterio.windows import Window
   00052: from rasterio.warp import transform as rasterio_transform
   00053: from rasterio.transform import xy as rasterio_xy
   00054: 
   00055: 
   00056: # =============================================================================
   00057: # USER SETTINGS
   00058: # =============================================================================
   00059: 
   00060: SFINCS_RUNS_ROOT = Path("/proj/zefflab/projects/Flooding/sfincs_runs")
>> 00061: EVENTS_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events")
   00062: 
   00063: # Kept exactly as requested, including spelling.
   00064: OUTPUT_DIR = Path("/proj/zefflab/projects/Flooding/Validation/Validation Outputs/raw_dem_depth_correlation_graphs")
   00065: 
   00066: # Raw DEM/topobathy surface to subtract from both observed and modeled WSE.
   00067: # This is the catalog DEM/topobathy file, not sfincs.dep.
   00068: RAW_DEM_PATH = Path(
   00069:     "/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/static/"
   00070:     "harris_county/topobathy/HCFCD_DEM_5m_UTM.tif"
   00071: )
   00072: 
   00073: # Validation point coordinates are expected to be UTM Zone 15N in this project.
```

#### Hit 2/2 — line 69 — `LIKELY_DEFAULT_OR_BROWSE`

Current line:

```text
"/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/static/"
```
...
>> 00069:     "/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/static/"
   ...
   00072: 
   00073: # Validation point coordinates are expected to be UTM Zone 15N in this project.
   00074: # If the DEM has a different CRS, rasterio will transform x/y before sampling.
   00075: VALIDATION_POINT_CRS = "EPSG:32615"
   00076: 
   00077: # If direct DEM sampling hits nodata, search nearby DEM pixels for the nearest finite value.
   00078: # At 5 m DEM resolution, 20 pixels is about 100 m.
   00079: RAW_DEM_NEAREST_FINITE_SEARCH_PIXELS = 20
   00080: 
   00081: # Keep False at first. Channel/river gauges can produce negative depth above raw DEM if
```
...
### `/proj/zefflab/projects/Flooding/Validation/Code/Data Catalog Validation Scripts/apply_harvey_3_boundary_catalog_repair.py`

Hits: **1**

#### Hit 1/1 — line 41 — `LIKELY_DEFAULT_OR_BROWSE`

Current line:

```text
EVENT_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events/harvey_2017_mrms")
```

Simple root replacement would become:

```text
EVENT_ROOT = Path("/users/e/p/epsilon/Data/Data/harris_county/catalogs/events/harvey_2017_mrms")
```
...
```text
   ...
   00031: # SAFETY SWITCH
   00032: # ---------------------------------------------------------------------
   00033: APPLY_CHANGES = False
   00034: 
   00035: 
   00036: # ---------------------------------------------------------------------
   00037: # Exact Harvey active files from audit
   00038: # ---------------------------------------------------------------------
   00039: EVENT_FOLDER = "harvey_2017_mrms"
   00040: 
>> 00041: EVENT_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events/harvey_2017_mrms")
   00042: 
   00043: BOUNDARY_ACTIVE = EVENT_ROOT / "event_boundary_table" / "event_boundary_table.csv"
   00044: WL_POINTS_ACTIVE = EVENT_ROOT / "event_waterlevel_points" / "noaa4_waterlevel_station_points.csv"
   00045: WL_ACTIVE = EVENT_ROOT / "event_waterlevel" / "harvey_houston_wl_bc.csv"
   00046: 
   00047: VALIDATION_OUT_ROOT = Path("/proj/zefflab/projects/Flooding/Validation/Validation Outputs/catalog_integrity")
   00048: 
   00049: TARGET_ACTIVE_IDS = ["8771013", "8770613", "08072050"]
   00050: MANCHESTER_ID = "8770777"
   00051: 
   00052: STATION_META = {
   00053:     "8771013": {
```
...
### `/proj/zefflab/projects/Flooding/Validation/Code/Data Catalog Validation Scripts/audit_14_event_boundary_point_maps.py`

Hits: **1**

#### Hit 1/1 — line 38 — `LIKELY_DEFAULT_OR_BROWSE`
...
```text
   ...
   00027: import pandas as pd
   00028: 
   00029: import matplotlib
   00030: matplotlib.use("Agg")
   00031: import matplotlib.pyplot as plt
   00032: 
   00033: 
   00034: # ---------------------------------------------------------------------
   00035: # Paths
   00036: # ---------------------------------------------------------------------
   00037: RUNS_ROOT = Path("/proj/zefflab/projects/Flooding/sfincs_runs")
>> 00038: EVENTS_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events")
   00039: OUT_ROOT = Path("/proj/zefflab/projects/Flooding/Validation/Validation Outputs/catalog_integrity")
   00040: 
   00041: # Adjust run_name only if one of these folders differs on disk.
   00042: EVENTS = [
   00043:     {
   00044:         "event": "ike_2008",
   00045:         "label": "Hurricane Ike",
   00046:         "run_name": "ike_2008_override_001",
   00047:         "tref": "2008-09-09 21:00:00",
   00048:         "tstart": "2008-09-09 21:00:00",
   00049:         "tstop": "2008-09-18 05:00:00",
   00050:     },
```


### `/proj/zefflab/projects/Flooding/Validation/Code/correlation_graph_generator.py`
...
```text
   ...
   00031: import pandas as pd
   00032: import xarray as xr
   ...
   00038: # =============================================================================
   00039: 
   00040: SFINCS_RUNS_ROOT = Path("/proj/zefflab/projects/Flooding/sfincs_runs")
>> 00041: EVENTS_ROOT = Path("/proj/zefflab/projects/Flooding/Data/harris_county/catalogs/events")
   00042: 
   00043: # Kept exactly as requested, including spelling.
   00044: OUTPUT_DIR = Path("/proj/zefflab/projects/Flooding/Validation/Validation Outputs/correlation_graphs")
   00045: 
   00046: 
   00047: '''
   00048:  "ike_2008_override_001",
   00049:  "flood_2009_04_override_005",
   00050:  c"flood_2012_01_override_001",
   00051:  "heavy_rain_2012_07_override_001",
   00052:  "heavy_rain_2014_08_override_001",
   00053:  "memorial_2015_override_001",
```
...
## B. Settings / path-root / browse-root chunks — full
...
- If Settings values changed but Browse starts did not, likely causes are:
  - Browse buttons read hardcoded fallbacks in HTML/JS.
  - Browse API in `app.py` has separate hardcoded roots.
  - `launcher_defaults.js` updates config defaults but `config_path_browse_buttons.js` opens paths independently.
  - `localStorage` settings are saved but not consumed by `openPathBrowser` / `startPath` code.
- Do not do global replace until chunks show which hits are true defaults.
- Desired eventual state: active code should not hardcode `/proj/.../Data`; it should use Settings/path-root helpers.

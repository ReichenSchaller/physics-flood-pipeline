...
def recommend_settings(level: str, run_count: int) -> Dict[str, Any]:
    ...
    if level == "quick":
        mem_gb = min(32, 8 + 2 * comparisons)
        cpus = 1
        time = "00:10:00"
        chunk_mb = 32
        note = "Quick compare avoids full physical array scans."

    elif level == "standard":
        mem_gb = min(128, 48 + 8 * comparisons)
        cpus = 2
        time = "00:45:00"
        chunk_mb = 128
        note = "Standard compare scans selected physical variables only."

    else:
        mem_gb = min(256, 96 + 16 * comparisons)
        cpus = 4
        time = "04:00:00"
        chunk_mb = 128
        note = "Full compare scans all overlapping numeric NetCDF variables. Use carefully."
...
util_note = {{
    ...
    "requested_cpus": request["slurm"]["cpus_per_task"],
    "max_pair_process_rss_gb": max_rss_gb,
    "approx_mem_utilization_percent": mem_util_pct,
    "note": "MaxRSS comes from /usr/bin/time -v for each Python compare process, not full Slurm job accounting.",
}}
